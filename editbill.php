...
if(!empty($_POST))
{
        ...
        $sql = "SELECT * FROM bill where billid= ? and party=?";

	// $sql = "SELECT count(*) FROM `table` WHERE foo = ?"; 
$var = $databasehandler->prepare($sql); 
session_start();
$var->execute([ $_POST['bln'],$_SESSION['party'] ]);
$v2 = $databasehandler->prepare($sql); 
// session_start();
$v2->execute([ $_POST['bln'],$_SESSION['party'] ]);
// echo "ok";    
...
<form action='add_debit_ud_data.php?serch=".$_comp."&blid=". $_POST['bln']."' enctype='multipart/form-data'  method='post'>
        ...
        $cnt=0;
         foreach ($var as $key) {
          ...
          echo "</select>
            ...
            <td><input required type='number' name='qty[]' value='".$key['qty']."' placeholder='Enter Qty' class='form-control qty' step='0' min='0'/></td>
            <td><input required type='text' name='price[]' value='".$key['price']."' placeholder='Enter Unit Price' class='form-control price' step='0.00' min='0'/></td>
            <td><input required type='number' name='total[]' value='".($key['price']*$key['qty'])."'  placeholder='0.00' class='form-control total' readonly/></td>
        </tr>";
